...
pub struct StateRootUpdateRecord {
    ...
    keccak256: u64,
    keccak256_execution: Keccak256Execution,
...
impl StateRootUpdateRecord {
    ...
    pub(crate) fn state_root_calculator(&self) -> u64 {
        self.state_root_calculator
    }
    ...
    pub(crate) fn keccak256_execution_count(&self) -> u64 {
        self.keccak256_execution.count
    }

    pub(crate) fn keccak256_execution_time(&self) -> u64 {
        self.keccak256_execution.time_cycles
    }
    ...
    pub(crate) fn add_state_root_calculator(&mut self, time: u64) {
        self.state_root_calculator =
            self.state_root_calculator.checked_add(time).expect("overflow");
    }
    ...
    pub(crate) fn add_keccak256(&mut self, time: u64) {
        self.keccak256 = self.keccak256.checked_add(time).expect("overflow");
    }
...
impl Drop for TimeRecorder {
    fn drop(&mut self) {
        ...
        match self.function {
            ...
            FunctionName::StateRootCalculator => {
                super::metric::record_state_root_calculator(time_cycles);
            }
...
impl TreeNode {
    ...
    pub fn new_leaf_only(leaf_number: u64, leaf_depth: u64) -> Self {
        Self {
            trie_node: Node::new_leaf_only(leaf_number, leaf_depth),
            state_node: Node::new_leaf_only(leaf_number, leaf_depth),
        }
    }
    ...
    pub fn add(&mut self, other: &Self) {
        self.trie_node.add(&other.trie_node);
        self.state_node.add(&other.state_node);
    }

    pub fn add_node(&mut self, other: &Self) {
        self.trie_node.add_node(&other.trie_node);
        self.state_node.add_node(&other.state_node);
    }
}
...
impl Node {
    ...
    pub fn new_leaf_only(leaf_number: u64, leaf_depth: u64) -> Self {
        Self { total_number: 0, leaf_number, total_depth: 0, leaf_depth }
    }
