...
static mut STRORAGE_ROOT_NODE: Option<perf_metrics::TreeNode> = None;
    ...
    H: HashedCursorFactory + Clone,
{
    ...
    pub fn root_with_updates(self) -> Result<(B256, TrieUpdates), StateRootError> {
        ...
        perf_metrics::record_changes(
            self.changed_account_prefixes.len() as u64,
            self.changed_storage_prefixes.len() as u64,
            self.changed_storage_prefixes.iter().map(|(_, v)| v.len() as u64).sum::<u64>(),
        );
    ...
    fn calculate(self, retain_updates: bool) -> Result<StateRootProgress, StateRootError> {
        ...
        perf_metrics::add_account_mpt_info(
            account_root_node,
            walker_updates.len().checked_add(self.destroyed_accounts.len()).expect("overflow")
                as u64,
